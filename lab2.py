...
def mutate(population, F):
    ...
    Для каждой особи в популяции создается мутантный вектор путем добавления взвешенной разницы между двумя случайно выбранными векторами к третьему вектору.
...
def crossover(mutant_population, population, CR):
    ...
    Для каждого вектора создается пробный вектор. Каждый элемент пробного вектора либо берется из мутантного вектора (с вероятностью CR), либо из целевого вектора."""
